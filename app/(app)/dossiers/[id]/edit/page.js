        ...
        <div className="space-y-4">
          <div className="bg-surface border border-border rounded-lg p-5">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={statut} onChange={(e) => setStatut(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]">
          ...
          <div className="bg-surface border border-border rounded-lg p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div><div className="text-xs text-sub">Frais sortie usine</div><input type="number" value={fraisSortieUsine} onChange={(e) => setFraisSortieUsine(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]" /></div>
              <div><div className="text-xs text-sub">Transport usine</div><input type="number" value={transportUsine} onChange={(e) => setTransportUsine(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]" /></div>
              <div><div className="text-xs text-sub">Cession Odoo / travaux ext.</div><input type="number" value={cessionOdoo} onChange={(e) => setCessionOdoo(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]" /></div>
              <div><div className="text-xs text-sub">Transport inter-site</div><input type="number" value={transportIntersite} onChange={(e) => setTransportIntersite(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]" /></div>
            </div>
          ...
          <div className="bg-surface border border-border rounded-lg p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              ...
              {financementActif && (
                <div><div className="text-xs text-sub">Montant financé</div><input type="number" value={financementMontant} onChange={(e) => setFinancementMontant(e.target.value)} className="w-full mt-1 border border-border rounded-md px-2 py-2 text-sm bg-[#FCFBF8]" /></div>
              )}
            </div>
            {financementActif && (
              <div className="mt-3">
        ...
        <div className="space-y-4">
          {calc && (
            <div className="bg-ink text-white rounded-lg p-5">
              <div className="text-[11px] text-[#B9C4CA] uppercase font-bold">Marge recalculée</div>
              <div className="flex gap-8 mt-2">
                <div><div className="text-[11px] text-[#B9C4CA]">Attendue</div><div className="text-2xl font-extrabold">{calc.destockage ? "Déstockage" : fmt(calc.E29)}</div></div>
                <div><div className="text-[11px] text-[#B9C4CA]">Réelle</div><div className="text-2xl font-extrabold" style={{ color: calc.E33 >= 0 ? "#8FD19E" : "#E39C97" }}>{fmt(calc.E33)}</div></div>
              </div>
              <div className="text-sm mt-3 text-[#B9C4CA]">Commission vendeur : <b className="text-white">{fmt(calc.commission)}</b></div>
            </div>
          )}
          <button onClick={enregistrer} disabled={saving} className="w-full py-2.5 rounded-md bg-accent text-white font-bold text-sm disabled:opacity-60">
